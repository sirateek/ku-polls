...
class QuestionDetailViewTests(TestCase):
    ...
    def test_past_question_not_accepting_the_vote(self):
        """
        Accessing detail view of a question in the pass which is stopped accepting
        ...
        index_response = self.client.get(response.url)
        self.assertContains(
            index_response, text=f"Question no. {past_question.id} is not accepting the vote anymore.", status_code=200)
